...
0. SÉCURITÉ ABSOLUE : Ne propose JAMAIS d'étapes, de quartiers, de routes ou de destinations classés dangereux, déconseillés aux voyageurs, ou sous alerte de sécurité (zones de conflit, criminalité élevée, instabilité politique). Si la destination globale est risquée, propose uniquement les zones sûres et mentionne-le dans country_tips.
...
- PRÉCISION DES PRIX RÉELS OBLIGATOIRE : utilise tes connaissances des prix moyens réels dans la VILLE ou RÉGION exacte demandée. Ex: hôtel 3★ à Rome en été = 120-180€/nuit (pas "60-90€"), hôtel 3★ à Lisbonne = 80-120€/nuit, hostel à Bangkok = 15-25€/nuit. Les prix doivent refléter la réalité du marché local, pas des fourchettes génériques. Pour chaque poste, explique le calcul dans budget_breakdown (ex: "Hôtel 3★ centre Rome × 5 nuits = 150€/nuit ÷ 2 personnes = 75€/nuit/personne").
...
FORMAT DE RÉPONSE OBLIGATOIRE (JSON pur, aucun markdown) :
{
  "logistics": {
    ...
    "packing_essentials": [
      "<item concret et ultra-personnalisé — RÈGLES STRICTES : (1) Adapté à la SAISON RÉELLE du voyage (ex: été → crème solaire SPF50, lunettes de soleil, vêtements légers ; hiver → manteau chaud, gants, chaussures imperméables). (2) Adapté au PAYS/RÉGION (ex: Japon → IC Card, adaptateur prise Type A ; UK → adaptateur prise Type G ; pays chaud → répulsif anti-moustiques). (3) Adapté au VÉHICULE (ex: van → kit cuisine, sac de couchage, galère rando ; moto → casque, gants moto, équipement pluie ; électrique → câble Type 2, app Chargemap). (4) Adapté aux COMPAGNONS (ex: famille → trousse premiers secours enfants, poussette légère si bébé ; solo → cadenas, power bank). (5) Adapté aux ACTIVITÉS choisies (ex: randonnée → bâtons, sac à dos 30L ; plage → serviette microfibre ; monuments → chaussures de marche). (6) Obligatoires légaux (ex: vignette autoroute, carte verte assurance, permis international). Génère 12-18 items, organisés du plus important au moins important.>",
      ...
    ],
...
- Type de voyage : ${destination_type === 'ville' ? 'VILLE (explorer une ville en profondeur — rester dans la ville et ses quartiers)' : destination_type === 'region' ? 'RÉGION (explorer une région spécifique — road trip dans un périmètre régional, pas tout un pays)' : 'PAYS (road trip / découverte du pays)'}
