...
${returnStr ? `- Retour : ${returnStr} — CONTRAINTE ABSOLUE SUR LES HORAIRES DU DERNIER JOUR :
  * Le voyageur doit arriver à "${return_place || 'son lieu de retour'}" AVANT ${return_time || "l'heure indiquée"}.
  * Calcule le trajet depuis la dernière activité jusqu'au lieu de retour et déduis l'heure limite de départ.
  * Si le retour se fait en avion : prévoir l'arrivée à l'aéroport AU MINIMUM 2h avant le vol pour un vol européen, 3h pour un vol international. Aucune activité ne doit être planifiée après l'heure de départ vers l'aéroport.
  * Si le retour se fait en train : prévoir 30 min avant le départ du train.
  * Le dernier jour doit uniquement contenir des activités COMPATIBLES avec cet horaire. Réduis le nombre d'étapes si nécessaire. Ne planifie jamais une activité dont la fin dépasse l'heure de départ obligatoire vers le lieu de retour.` : ''}
